...
class ODSExtractor implements APIExtractor {

    private static $queryNext2WeeksEvents = 'https://public.opendatasoft.com/api/records/1.0/search/?dataset=evenements-publics-cibul&q=date_start+%3E+%23now()+AND+date_start+%3C%3D+%23now(weeks%3D%2B2)&rows=5000';
    ...
    {
        $this->getRecordsFromApi(ODSExtractor::$queryNext2WeeksEvents);
...
*/
    ...
    private function createEventFields(array $fields): Event
    {
        ...
        $event = $eventBuilder
            ...
            ->addAddressName($fields['placename'])
            ->addAddressCity($fields['city'])
            ->addAddressStreet($fields['address'])
